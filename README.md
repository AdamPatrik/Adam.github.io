...
<body>
  ...
  <script>
    ...
    form.addEventListener('submit', async (ev) => {
      ...
      // Try server-first (if your backend exists). Expects JSON { email, password, returnTo } -> returns { success, redirectTo? }
      try {
        ...
        // No server available: fall back to client-side demo auth
        console.warn('Server login failed or not available, using client-side demo check.', serverErr);
      }

      // CLIENT-SIDE DEMO login (localStorage)
      try {
